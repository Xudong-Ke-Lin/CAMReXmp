 ...
 * Pure Euler tests
 * -----------------------------------------------------------*/
  if (test=="Toro1" || test=="Toro2" || test=="Toro3" || test=="Toro4" || test=="Toro5"){
 ...
 * -----------------------------------------------------------*/
    ...
    // two-fluid Brio-Wu version of the cylindrical explostion
  } else if (test=="BrioWu2DCart"){
    ...
    // two-fluid Brio-Wu version of cylindrical explosion in 1D
  } else if (test=="BrioWu1DCyl"){
    Real rho_in = 1.0, v_x_in = 0.0, v_y_in = 0.0, v_z_in = 0.0, p_in = 0.5;
    Real rho_out = 0.125, v_x_out = 0.0, v_y_out = 0.0, v_z_out = 0.0, p_out = 0.05;
		...
		for(int i = lo.x; i <= hi.x; i++)
		  {
		    ...
		    */
		    ...
		    Real bessel0k = std::real(std::cyl_bessel_j(0,k0));		   
		    Real bessel0rk = std::real(std::cyl_bessel_j(0,k0*rCyl));
		    Real bessel1rk = std::real(std::cyl_bessel_j(1,k0*rCyl));
		    Real bessel2rk = std::real(std::cyl_bessel_j(2,k0*rCyl));
		    Real bessel3rk = std::real(std::cyl_bessel_j(3,k0*rCyl));

		    // original variables
		    Real psi0, I0, p_e0;
		    // derived variables
		    Real B_r, B_theta, J_r, J_theta, J_z;
		    
		    if (rCyl<=1.0){
		      // flux
		      psi0 = (2./(k0*bessel0k))*bessel1rk*costh;
		      // toroidal field
		      I0 = std::sqrt((1.-beta)*k0*k0*psi0*psi0+B0*B0);
		      // electron pressure
		      p_e0 = 0.5*k0*k0*beta*psi0*psi0+0.01;
		      // magnetic field B = (Grad Psi) x z
		      B_r = -2.*bessel1rk*sinth/(k0*rCyl*bessel0k);
		      B_theta = -(bessel0rk-bessel2rk)*costh/(bessel0k);
		      // current J = Curl B
		      J_r = 2.*(beta-1.)*bessel1rk*bessel1rk*std::sin(2.*theta)/(rCyl*bessel0k*bessel0k*I0);
		      J_theta = 2.*k0*(beta-1.)*bessel1rk*(bessel0rk-bessel2rk)*costh*costh/(bessel0k*bessel0k*I0);
		      J_z = (-2.*k0*rCyl*bessel0rk+(4.+3.*k0*k0*rCyl*rCyl)*bessel1rk+k0*rCyl*(2.*bessel2rk-k0*rCyl*bessel3rk))*costh/(2.*k0*rCyl*rCyl*bessel0k);
		    } else{
		      psi0 = (rCyl-1./rCyl)*costh;
		      I0 = B0;
		      p_e0 = 0.01;

		      B_r = -sinth+sinth/(rCyl*rCyl);
		      B_theta = -costh-costh/(rCyl*rCyl);
		      J_r = 0.0, J_theta = 0.0, J_z = 0.0;

		    }
		    B_x = B_r*costh - B_theta*sinth;
		    B_y = B_r*sinth + B_theta*costh;
		    B_z = I0;
		    Real J_x = B_r*costh - B_theta*sinth;
		    Real J_y = B_r*sinth + B_theta*costh;		    
		      
		    arr(i,j,k,MOMX_E) = J_x;
		    arr(i,j,k,MOMY_E) = J_y;
		    arr(i,j,k,MOMZ_E) = J_z;
		    arr(i,j,k,BX) = B_x;
		    arr(i,j,k,BY) = B_y;
		    arr(i,j,k,BZ) = B_z;
		    //arr(i,j,k,DIVB) = psi0;
		    ...
		    arr(i,j,k,DIVE) = 0.0;
    		    ...
    		    rho = 1.0, v_x = 0.0, v_y = 0.0, v_z = 0.0;
		    // perturbation
		    v_x = -0.001*std::exp(-rCyl*rCyl)*std::sin(y);
    		    Real p_i0 = 0.0;
    		    ...
    		    arr(i,j,k,2) = arr(i,j,k,0)*v_y;
    		    arr(i,j,k,3) = arr(i,j,k,0)*v_z;
    		    Vector<Real> w_i{rho, v_x, v_y, v_z, p_i0+p_e0, rho/m, J_x, J_y, J_z, p_e0};
    		    arr(i,j,k,ENER_I) = get_energy_totalGOL(w_i);
		    
    		    arr(i,j,k,RHO_E) = rho/m;
    		    Vector<Real> w_e{rho/m, v_x_e, v_y_e, v_z_e, p_e0};
    		    arr(i,j,k,ENER_E) = get_energy(w_e);
    ...
    // 		    rho = 1.0, v_x = 0.0, v_y = 0.0, v_z = 0.0;
    // 		    Real p_i = 0.0;
    // 		    Real p_e = arr(i,j,k,DIVE);

    // 		    // quasi-neutrality
    // 		    Real n_e = rho;
    // 		    Real q_i = r_i;
		    
    // 		    /*Real Psi0 = arr(i,j,k,DIVB);
    // 		    Ral dxPsi = computeDerivative(arr(i-1,j,k,DIVB),arr(i+1,j,k,DIVB),dX);
    // 		    Real dyPsi = computeDerivative(arr(i,j-1,k,DIVB),arr(i,j+1,k,DIVB),dY);
    // 		    Real dxxPsi = computeSecondDerivative(arr(i-1,j,k,DIVB),arr(i,j,k,DIVB),arr(i+1,j,k,DIVB),dX);
    // 		    Real dyyPsi = computeSecondDerivative(arr(i,j-1,k,DIVB),arr(i,j,k,DIVB),arr(i,j+1,k,DIVB),dY);
    // 		    Real I0 = arr(i,j,k,BZ);
    // 		    Real dxPe = computeDerivative(arr(i-1,j,k,DIVE),arr(i+1,j,k,DIVE),dX);
    // 		    Real dyPe = computeDerivative(arr(i,j-1,k,DIVE),arr(i,j+1,k,DIVE),dY);
    // 		    */
    // 		    // magnetic field B = (Grad Psi) x z + I0 z 
    // 		    //B_x = dyPsi, B_y = -dxPsi, B_z = I0;
    // 		    // current J = Curl B
    // 		    Real J_x = k0*k0*(1.-beta)*Psi0*dyPsi/I0;
    // 		    Real J_y = -k0*k0*(1.-beta)*Psi0*dxPsi/I0;
    // 		    Real J_z = -dxxPsi-dyyPsi;
    // 		    // electric field E = -v x B + J x B/ne - Grad p_e/ne
    // 		    Vector<Real> vxB = cross_product({v_x,v_y,v_z},{B_x,B_y,B_y});
    // 		    Vector<Real> JxB = cross_product({J_x,J_y,J_z},{B_x,B_y,B_y});
    // 		    Real E_x = -vxB[0] + JxB[0]/(n_e*q_i) - dxPe/(n_e*q_i);
    // 		    Real E_y = -vxB[1] + JxB[1]/(n_e*q_i) - dyPe/(n_e*q_i);
    // 		    Real E_z = -vxB[2] + JxB[2]/(n_e*q_i);
    ...
    // 		    arr(i,j,k,2) = arr(i,j,k,0)*v_y;
    // 		    arr(i,j,k,3) = arr(i,j,k,0)*v_z;
    // 		    Vector<Real> w_i{rho, v_x, v_y, v_z, p_i+p_i, rho/m, J_x, J_y, J_z, p_e};
    // 		    arr(i,j,k,ENER_I) = get_energy_totalGOL(w_i);
	    
    // 		    arr(i,j,k,RHO_E) = rho/m;
    // 		    arr(i,j,k,MOMX_E) = J_x;
    // 		    arr(i,j,k,MOMY_E) = J_x;
    // 		    arr(i,j,k,MOMZ_E) = J_z;
    // 		    Vector<Real> w_e{rho/m, v_x_e, v_y_e, v_z_e, p_e};
    // 		    arr(i,j,k,ENER_E) = get_energy(w_e);
