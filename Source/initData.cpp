  ...
  */
  ...
  for (MFIter mfi(S_EM_X); mfi.isValid(); ++mfi)
  {
	...
	for(int i = lo.x; i <= hi.x; i++)
	{
	  ...
	  } else if (test=="OTAmano"){
	    v_x = -0.5*std::sin(2.0*M_PI*y);
	    v_y = 0.5*std::sin(2.0*M_PI*x);
	    v_z = 0.0;	    
	    B_x = -std::sin(2.0*M_PI*y)/std::sqrt(4.0*M_PI);
	    B_y = std::sin(4.0*M_PI*x)/std::sqrt(4.0*M_PI);
	    B_z = 0.0;
	    Vector<Real> vcrossB = cross_product({v_x,v_y,v_z},{B_x,B_y,B_z});	    
  ...
  for (MFIter mfi(S_new); mfi.isValid(); ++mfi)
  {
	...
	for(int i = lo.x; i <= hi.x; i++)
	{
	  ...
	  if (test=="BrioWu"){
	    ...
	    //Real rho_in = 5e-3, p_in = 0.5, rho_out = 5e-5, p_out = 2.5e-4;
	    //rho = 1.0, v_x = 0.0, v_y = 0.0, v_z = 0.0;
	    //Real p_in = 1000.0, p_out = 0.1;
	    ...
	    if (x*x+y*y <= 0.8*0.8){
	      rho = rho_in, p = p_in;
	    } else if (x*x+y*y > 0.8*0.8 && x*x+y*y < 1.0){
	      rho = rho_in+(rho_out-rho_in)/0.2*(std::sqrt(x*x+y*y)-0.8);
	      p = p_in+(p_out-p_in)/0.2*(std::sqrt(x*x+y*y)-0.8);
	    } else{
	      rho = rho_out, p = p_out;
	    }
	    */
