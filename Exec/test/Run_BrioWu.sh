...
do
    ...
    mpiexec -n $cpu /data/tycho/xk215/MPHIL_thesis/make_models/$model/./CAMREXmp1d.gnu.MPI.ex inputs l_r=$l_r amr.n_cell=${array[i]}" 10 128" adv.cfl=${array2[i]}  amr.plot_file="/data/atalanta/xk215/MPHIL_thesis/"$model"/plt_BrioWu_WENOMUSCLfull_lr"$l_r"_n_cell"${array[i]}"_cfl"${array2[i]}"_"
done
for f in "/data/atalanta/xk215/MPHIL_thesis/"$model"/plt_BrioWu_WENOMUSCLfull_lr"$l_r"_n_cell"*"/"; do ~/AMReX/amrex/Tools/Plotfile/./fextract.gnu.ex $f; done
